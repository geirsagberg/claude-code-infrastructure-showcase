...
- Useful if projects need different hook versions
...
**How it works:**
1. Reads `skill-rules.json` from current project
...
**Integration (Global - Recommended):**
```bash
# Copy both files to global hooks
...
**Add to global settings (~/.claude/settings.json):**
```json
{
  "hooks": {
    "UserPromptSubmit": [
...
**Add to project settings (your-project/.claude/settings.json):**
```json
{
  "hooks": {
    "UserPromptSubmit": [
...
**Customization:** ✅ None needed - reads project-specific skill-rules.json automatically
...
3. Creates cache in project's `.claude/tsc-cache/`
